...
class GueTask {
  ...
  beginTask() {
    ...
    gueEvents.emit('GueTask.beginTask', this);
  }
  ...
  endTask() {
    ...
    gueEvents.emit('GueTask.endTask', this);
  }

  /**
   * getTaskDuration - Returns how long the task ran
   *
   * @returns {integer} Number of ms the task ran
   */
  getTaskDuration() {
    let retVal = 0;
    if (this.endTime > 0) {
   ...
   *
   * @returns {integer} Number of ms the action ran
   */
  getActionDuration() {
    let retVal = 0;
  ...
  runAction() {
    ...
    .catch((val)=> {
      gueEvents.emit('GueTask.runAction.error', val);
      throw val;
    });
  }
}
