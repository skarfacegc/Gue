...
class GueTasks {
  ...
  addEventHandlers(task) {
    this.tasks[task].on('taskExecuteStart', ()=> {
      console.log(task + ' execute start');
    });

    this.tasks[task].on('taskExecuteEnd', ()=> {
      console.log(task + ' execute done');
    });
