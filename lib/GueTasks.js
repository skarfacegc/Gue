...
class GueTasks {
  ...
  runTask(taskName) {
    ...
    // This is the function passed to the reduce call on the list
    // of dependencies
    let runDep = (promise, cur) => {
    ...
    // base case, no dependencies, run the task
    if (!this.tasks[taskName].hasDependencies()) {
      retP = this.tasks[taskName].execute();

    // Process the dependencies, then execute taskName
    } else {
      retP = this.tasks[taskName].dependencies.reduce(runDep, Promise.resolve())
      .then(() => {
